...
import { HealthndFitness } from '../../../../../types';
...
export default async function BlogPostPage({ params }: BlogPostPageProps) {
  ...
  const articleQuery = `*[_type == "health-and-fitness" && slug.current == $slug][0] {
    title,
    overview,
    mainImage {
      asset,
  ...
  const relatedTopicsQuery = `*[_type == "health-and-fitness" && slug.current != $slug] | order(_createdAt desc)[0...5] {
    title,
    slug {
      current
    },
          ...
          {relatedTopics.map((topic) => (
            <li key={topic.slug.current} className="mb-4">
              <div className="ml-4">
                ...
                <Link href={`/health-and-fitness/${topic.slug.current}`} className="text-blue-600 hover:underline text-lg font-semibold">
                  {topic.title}
                </Link>
