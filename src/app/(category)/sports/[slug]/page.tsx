...
export default async function BlogPostPage({ params }: BlogPostPageProps) {
  ...
  const relatedTopics: Array<Sports> = await client.fetch(relatedTopicsQuery, { slug });
          ...
          {relatedTopics.map((topic) => (
            <li key={topic.slug.current} className="mb-4">
              <div className="ml-4">
                ...
                <Link href={`/sports/${topic.slug.current}`} className="text-blue-600 hover:underline text-lg font-semibold">
                  {topic.title}
                </Link>
