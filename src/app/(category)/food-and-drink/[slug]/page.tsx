...
import { FoodandDrink } from '../../../../../types';
...
export default async function BlogPostPage({ params }: BlogPostPageProps) {
  ...
  const articleQuery = `*[_type == "food-and-drink" && slug.current == $slug][0] {
    title,
    overview,
    mainImage {
      asset,
  ...
  const relatedTopicsQuery = `*[_type == "food-and-drink" && slug.current != $slug] | order(_createdAt desc)[0...5] {
    title,
    slug {
      current
    },
  ...
  const article: FoodandDrink | null = await client.fetch(articleQuery, { slug });
  const relatedTopics: Array<FoodandDrink> = await client.fetch(relatedTopicsQuery, { slug });
          ...
          {relatedTopics.map((topic) => (
            <li key={topic.slug.current} className="mb-4">
              <div className="ml-4">
                ...
                <Link href={`/food-and-drink/${topic.slug.current}`} className="text-blue-600 hover:underline text-lg font-semibold">
                  {topic.title}
                </Link>
